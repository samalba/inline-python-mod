...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "InlinePython":
		...
		case "WithPackage":
			...
			return (*InlinePython).WithPackage(&parent, ctx, name), nil
		case "WithPackages":
			var err error
			var parent InlinePython
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*InlinePython).WithPackages(&parent, ctx, packages), nil
		case "NoCache":
			var err error
			var parent InlinePython
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*InlinePython).NoCache(&parent, ctx), nil
		case "Code":
			var err error
			var parent InlinePython
			err = json.Unmarshal(parentJSON, &parent)
		...
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("InlinePython").
					WithFunction(
						dag.NewFunction("WithPackage",
							dag.TypeDef().WithObject("InlinePython")).
							WithArg("name", dag.TypeDef().WithKind(Stringkind))).
					WithFunction(
						dag.NewFunction("WithPackages",
							dag.TypeDef().WithObject("InlinePython")).
							WithArg("packages", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(Stringkind)))).
					WithFunction(
						dag.NewFunction("NoCache",
							dag.TypeDef().WithObject("InlinePython"))).
					WithFunction(
						dag.NewFunction("Code",
							dag.TypeDef().WithObject("Container")).
							WithArg("code", dag.TypeDef().WithKind(Stringkind))).
					WithField("Ctr", dag.TypeDef().WithObject("Container"), TypeDefWithFieldOpts{Description: ""})), nil
	default:
